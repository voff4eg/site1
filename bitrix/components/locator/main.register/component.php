...
// register user
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["UF_GET_EMAIL"] == "on" && $_REQUEST["UF_GET_SMS"] == "on" && $_REQUEST["UF_GET_COURSE"] == "on" && !$USER->IsAuthorized())
{
	...
	$_REQUEST["PERSONAL_BIRTHDATE"] = "";
	$_REQUEST["WORK_STREET"] = "";
	...
	{
		if ($key != "PERSONAL_PHOTO" && $key != "WORK_LOGO" && $key != "WORK_STREET" && $key != "PERSONAL_BIRTHDATE")
		{
			$arResult["VALUES"][$key] = $_REQUEST[$key];
	...
	if (strlen($_REQUEST["STREET"]) > 0) 
		$work_street .= $_REQUEST["STREET"].",";
		
	if (strlen($_REQUEST["HOUSER"]) > 0) 
		$work_street .= $_REQUEST["HOUSE"].",";
		
	if (strlen($_REQUEST["BLD"]) > 0) 
		$work_street .= $_REQUEST["BLD"].",";
		
	if (strlen($_REQUEST["FLAT"]) > 0) 
		$work_street .= $_REQUEST["FLAT"].",";

	$work_street = substr($work_street, 0, strlen($work_street)-1);	
	
	$arResult["VALUES"]["WORK_STREET"] = $work_street; 

	// день рождения
	$birthdate_str = "";	

	//if ($_REQUEST["BIRTHDAY-DAY"] < 10)   $_REQUEST["BIRTHDAY-DAY"]   = '0'.$_REQUEST["BIRTHDAY-DAY"];
	if ($_REQUEST["BIRTHDAY-MONTH"] < 10) $_REQUEST["BIRTHDAY-MONTH"] = '0'.$_REQUEST["BIRTHDAY-MONTH"];
	
	$birthdate_str = $_REQUEST["BIRTHDAY-DAY"].".".$_REQUEST["BIRTHDAY-MONTH"].".".$_REQUEST["BIRTHDAY-YEAR"];
						
	$arResult["VALUES"]["PERSONAL_BIRTHDAY"] = $birthdate_str;
	...
	// галочки в форме
	if ($_REQUEST["UF_GET_EMAIL"] == "on") $arResult["VALUES"]["UF_GET_EMAIL"] = "Y";
	if ($_REQUEST["UF_GET_SMS"]  == "on") $arResult["VALUES"]["UF_GET_SMS"] = "Y";
	if ($_REQUEST["UF_GET_COURSE"] == "on") $arResult["VALUES"]["UF_GET_COURSE"] = "Y";
	...
	else // if there;s no any errors - create user
	{
		...
		if (intval($ID) > 0)
		{
			...
			$arResult["VALUES"]["UF_GET_EMAIL"] = "Y";
			$arResult["VALUES"]["UF_GET_SMS"] = "Y";
			$arResult["VALUES"]["UF_GET_COURSE"] = "Y";
			
			$arUpdate = array("ACTIVE" => "N"/*, "UF_GET_EMAIL" => array('Y'), "UF_GET_SMS" => array('Y'), "UF_GET_COURSE" => array('Y')*/);
			$user->Update($ID, $arUpdate);
			
			/*$arUpd = array("UF_GET_EMAIL" => 'Y', "UF_GET_SMS" => 'Y', "UF_GET_COURSE" => 'Y');
			$GLOBALS["USER_FIELD_MANAGER"]->Update("USER", $ID, $arUpd);*/
		}
		else
		{
