...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	$arFilter = Array("GROUPS_ID" => Array(6));	
	//$arFields["FIELDS"] = Array("ID", "LOGIN", "NAME", "LAST_NAME", "SECOND_NAME", "WORK_POSITION", "PERSONAL_STREET","PERSONAL_PHOTO");
	//$arFields["SELECT"] = Array("UF_EXPERIENCE");
	//$arFields["NAV_PARAMS"] = Array("nPageSize" => 6);
	
	$rsUsers = CUser::GetList(($by="name"), ($order="desc"), $arFilter);
	while ($arUser = $rsUsers->GetNext()) {
		$arUser["DETAIL_PAGE_URL"] = "/doctor/?ID=".$arUser["ID"];
		if (strlen(trim($arUser["UF_EXPERIENCE"])) > 0){
			$arUser["UF_EXPERIENCE"] .= "Стаж ".$arUser["UF_EXPERIENCE"];
		}
