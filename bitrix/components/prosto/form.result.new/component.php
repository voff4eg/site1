...
{
        //$arValues = CFormResult::GetDataByIDForHTML(1, "Y");
	//echo "<pre>"; print_r($arValues); echo "</pre>";die;
...
/*************************************************************************************************/
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen($_REQUEST["web_form_submit"])>0 || strlen($_REQUEST["web_form_apply"])>0))
		{
			...
			//echo "<pre>"; print_r($_REQUEST); echo "</pre>";
			//echo "<pre>"; print_r($arResult["FORM_ERRORS"]); echo "</pre>";
			if (
			...
			)
			{
				...
				if (check_bitrix_sessid())
				{
					...
					if($RESULT_ID = CFormResult::Add($arParams["WEB_FORM_ID"], $arResult["arrVALUES"]))
					{
						...
						CEvent::Send("REGISTRATION_REQUEST", SITE_ID, array("EMAIL"=>$_REQUEST["form_email_2"],"PHONE"=>$_REQUEST["form_text_1"]),25);
						// send email to manager	
						CEvent::Send("REGISTRATION_REQUEST", SITE_ID, array("EMAIL"=>$_REQUEST["form_email_2"],"PHONE"=>$_REQUEST["form_text_1"]),26);
