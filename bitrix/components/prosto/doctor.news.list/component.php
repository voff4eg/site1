...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$arSelect = array_merge($arParams["FIELD_CODE"], array(
			...
			"PREVIEW_TEXT_TYPE",
			"PREVIEW_PICTURE",
			"PROPERTY_howstart_on_mainpage",
			"PROPERTY_novosti_on_mainpage",
			"PROPERTY_vmire_on_mainpage",
			"PROPERTY_howstart_TISER",
			"PROPERTY_novosti_TISER",
			"PROPERTY_vmire_TISER",
			"CREATED_DATE",
		));
		...
		while($obElement = $rsElement->GetNextElement())
		{
			$arItem = $obElement->GetFields();
			 ...
			 if ($arItem["PROPERTY_HOWSTART_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_HOWSTART_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			 if ($arItem["PROPERTY_NOVOSTI_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_NOVOSTI_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			 if ($arItem["PROPERTY_VMIRE_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_VMIRE_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
