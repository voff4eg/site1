...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$arSelect = array_merge($arParams["FIELD_CODE"], array(
			...
			"PREVIEW_TEXT_TYPE",
			"PREVIEW_PICTURE",
			"PROPERTY_learn_on_mainpage",
			"PROPERTY_find_on_mainpage",
			"PROPERTY_story_on_mainpage",
			"PROPERTY_advice_on_mainpage",
			"PROPERTY_woman_on_mainpage",
			"PROPERTY_LEARN_TISER",
			"PROPERTY_FIND_TISER",
			"PROPERTY_STORY_TISER",
			"PROPERTY_ADVICE_TISER",
			"PROPERTY_WOMAN_TISER",
			"CREATED_DATE",
		));
		//echo $bGetProperty;
		...
		if(intval($arResult["PICTURE"]) > 0){
			$detPic = CFile::GetByID($arResult["PICTURE"]);
			$arPic = $detPic->Fetch();
			$arResult["SECTION"]["SECTION_PREVIEW_PICTURE"]["SRC"] = "/upload/".$arPic["SUBDIR"]."/".$arPic["FILE_NAME"];
			$arResult["SECTION"]["SECTION_PREVIEW_PICTURE"]["WIDTH"] = $arPic["WIDTH"];
			$arResult["SECTION"]["SECTION_PREVIEW_PICTURE"]["HEIGHT"] = $arPic["HEIGHT"];
			//$arResult["SECTION"]["SECTION_PREVIEW_PICTURE"] = CFile::GetPath($arResult["PICTURE"]);
		}
		...
		while($obElement = $rsElement->GetNextElement())
		{
			$arItem = $obElement->GetFields();
			 ...
			 if ($arItem["PROPERTY_LEARN_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_LEARN_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			 if ($arItem["PROPERTY_FIND_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_FIND_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			 if ($arItem["PROPERTY_STORY_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_STORY_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			 if ($arItem["PROPERTY_ADVICE_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_ADVICE_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			 if ($arItem["PROPERTY_WOMAN_TISER_VALUE"] != "") {
				$arItem_tiser = CFile::GetByID($arItem["PROPERTY_WOMAN_TISER_VALUE"]);
				$arItem["TISER"] = $arItem_tiser->Fetch();
				$arItem["TISER"]["SRC"] = "/upload/".$arItem["TISER"]["SUBDIR"]."/".$arItem["TISER"]["FILE_NAME"];
			 }
			...
			// Динамические изображения в разделах
			$res2 = CIBlockSection::GetByID($arItem["IBLOCK_SECTION_ID"]);
			if($ar_res = $res2->GetNext()) {
					// echo "<pre>";print_r($ar_res);echo "</pre>";
					$arItem["IBLOCK_SECTION_NAME"] = $ar_res["NAME"];
					$arItem["SECTION_PAGE_URL"]    = $ar_res["SECTION_PAGE_URL"];
				
					$detPic = CFile::GetByID($ar_res["PICTURE"]);
					$arPic = $detPic->Fetch();
					$arResult["SECTION_PREVIEW_PICTURE"] = $arPic;
					// echo "<pre>";print_r($arItem["SECTION_PREVIEW_PICTURE"]);echo "</pre>";
				}
