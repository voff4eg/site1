		...
		<div class="b-popup-window" data-doctor="<?=$arUser["ID"]?>">
			...
			<div class="b-popup-window_theme-1 b-popup-window_type-sign-in">
				...
				<h2 class="b-popup-window__heading">
					<?if(strlen(trim($arUser["SECOND_NAME"])) > 0):?><div class="b-doctor-page__lastname"><?=trim($arUser["SECOND_NAME"])?></div><?endif;?>
					<?if(strlen(trim($arUser["LAST_NAME"])) > 0 || strlen(trim($arUser["NAME"])) > 0):?><div class="b-doctor-page__name"><?if(strlen(trim($arUser["NAME"])) > 0):?><?=trim($arUser["NAME"])?><?if(strlen(trim($arUser["LAST_NAME"])) > 0):?> <?=trim($arUser["LAST_NAME"])?><?endif;?><?else:?><?=trim($arUser["LAST_NAME"])?><?endif;?></div><?endif;?>
				...
				<div class="b-popup-window__note">
					<a href="mailto:">Сообщите нам</a>, если вам не удалось связаться с доктором
				</div>
