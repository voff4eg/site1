				...
				else
				{
					if ($arItem["SELECTED"]){
						...
						<li class="b-top-submenu__item b-top-submenu__item__<?=$arItem["PARAMS"]["image"]?> b-top-submenu__item__current">							
							<span class="b-menu-item b-top-submenu__item__text i-image">
								<span class="i-image__text"><?=$arItem["TEXT"]?></span>
							</span>
						</li>
						<?
					}else{
						//if($arItem["LINK"] != $curdir){?>
							<!--<li class="b-top-submenu__item b-top-submenu__item__<?=$arItem["PARAMS"]["image"]?> b-top-submenu__item__current">
								<a class="b-menu-item b-top-submenu__item__text i-image" href="<?=$arItem["LINK"]?>">
									<span class="i-image__text"><?=$arItem["TEXT"]?></span>
								</a>
							</li>-->
						<?//}else{?>
					...
					if ($arItem["DEPTH_LEVEL"] == 1)
					{
						if ($arItem["SELECTED"])
						{
							...
							<li class="b-top-menu__item b-top-menu__item__<?=$arItem["PARAMS"]["image"]?> b-top-menu__item__current">
                                <span class="b-menu-item b-top-submenu__item__text i-image"><span class="i-image__text"><?=$arItem["TEXT"]?></span></span>
                            </li>
							<?
						}
						else
